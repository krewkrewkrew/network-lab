/**
 * VyOS Simulator page
 * Wraps the VyOS-specific Terminal + TrainingPanel.
 * Terminal/TrainingPanel components must be copied from krewkrewkrew/vyos-lab
 * into src/components/vyos/ and src/lib/vyos/
 */
import React, { useState, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import { PanelLeftClose, PanelLeftOpen, ArrowLeft } from 'lucide-react';
import Terminal from '@/components/vyos/terminal/Terminal';
import StatusBar from '@/components/vyos/terminal/StatusBar';
import TrainingPanel from '@/components/vyos/training/TrainingPanel';
import { createDefaultSwitchState } from '@/lib/vyos/switchState';

export default function VyOSSimulator() {
    ...
    <div className="h-screen w-screen flex flex-col overflow-hidden" style={{ backgroundColor: '#0D1117' }}>
      <div className="flex items-center justify-between px-4 py-2 border-b border-slate-800/60" style={{ backgroundColor: '#060a12' }}>
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-slate-400 font-mono">VyOS Network Simulator · Advanced Technological Solutions</span>
        </div>
